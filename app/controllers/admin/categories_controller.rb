class Admin::CategoriesController < AdminController
  ...
  def index
    @admin_categories = Category.all.order(created_at: :desc)
  end
  ...
  def update
    if @admin_category.update(admin_category_params.reject { |k| k["image"] })
      if admin_category_params["image"]
        @admin_category.image.attach(image)
      end
  ...
  private
    ...
    def admin_category_params
      params.require(:category).permit(:name, :description, :parent_id, :image)
    end
end
